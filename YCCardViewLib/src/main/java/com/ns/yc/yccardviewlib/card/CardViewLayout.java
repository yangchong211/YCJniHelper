...
public class CardViewLayout extends ViewGroup {
    ...
    private boolean hasSetAdapter = false;                  // 是否设置adapter
    ...
    public CardViewLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.cardView);
        interval = (int) a.getDimension(R.styleable.cardView_interval, interval);
        sizeRatio = a.getFloat(R.styleable.cardView_sizeRatio, sizeRatio);
        scaleStep = a.getFloat(R.styleable.cardView_scaleStep, scaleStep);
        displayCount = a.getFloat(R.styleable.cardView_displayCount, displayCount);
        a.recycle();
    ...
    private void doBindAdapter() {
        ...
        if (hasSetAdapter) {
            throw new RuntimeException("CardViewLayout can only hold one adapter.");
        }
        hasSetAdapter = true;
        if (getChildCount() == 0) {
